...
const toothTargets = [
  { id: 't-1', x: 30, y: 60, w: 18, h: 22 },
];

const bleedingAreas = [
  { id: 'b-1', x: 30, y: 62, w: 20, h: 22 },
];
...
const buildState = (areas, initial = true) => areas.reduce((acc, area) => ({ ...acc, [area.id]: initial }), {});

const isInside = (x, y, area) => x >= area.x && x <= area.x + area.w && y >= area.y && y <= area.y + area.h;

const Patient3Extract = ({ onComplete }) => {
  const stageRef = useRef(null);
  const [phase, setPhase] = useState('intro');
  const [status, setStatus] = useState('Gunakan kaca mulut untuk memeriksa gusi Aru.');
  ...
  const [numbState, setNumbState] = useState(() => buildState(soreAreas, false));
  const [toothState, setToothState] = useState(() => buildState(toothTargets, true));
  const [bleedState, setBleedState] = useState(() => buildState(bleedingAreas, true));
  const [bacteriaState, setBacteriaState] = useState(() => buildState(bacteriaAreas, true));

  const remainingSore = useMemo(() => Object.values(soreState).filter(Boolean).length, [soreState]);
  const remainingTooth = useMemo(() => Object.values(toothState).filter(Boolean).length, [toothState]);
  const remainingBleed = useMemo(() => Object.values(bleedState).filter(Boolean).length, [bleedState]);
  const remainingBacteria = useMemo(() => Object.values(bacteriaState).filter(Boolean).length, [bacteriaState]);
  ...
  const stageClassName = useMemo(() => {
    ...
    if (draggingTool === 'mirrorProbe' && !completed.mirrorProbe) base.push('target-mirror');
    if (hoveringStage && draggingTool === 'mirrorProbe' && !completed.mirrorProbe) base.push('is-hover');
    return base.join(' ');
  ...
  useEffect(() => {
    ...
    const handleMove = (event) => {
      ...
      if (draggingTool === 'anesthesia') {
        ...
        if (numbed) setStatus('Bius bekerja. Lanjutkan pada area bengkak lainnya.');
        ...
        if (pulled) setStatus('Gigi berhasil dicabut! Bersihkan darah dengan penyedot.');
        ...
        if (pressed) setStatus('Perdarahan berhenti. Angkat bakterinya dengan pinset.');
        ...
        if (removedSomething) setStatus('Bakteri terangkat! Bersihkan semua sebelum lanjut.');
      }
    };
  ...
  }, [draggingTool, phase, soreState, toothState, bleedState, bacteriaState, completed.mirrorProbe]);
  ...
  const activeTool = useMemo(() => toolSequence.find((tool) => tool.id === draggingTool), [draggingTool, toolSequence]);
  ...
  const allDone = completed.mirrorProbe && completed.anesthesia && completed.extract && completed.suction && completed.gauze && completed.tweezers;

  return (
    <div className="patient3-layout">
      <div className="patient3-info">
        <h2>Aru - Pencabutan Gigi</h2>
        <p className="patient3-complaint">"Gusi aku sakit dan giginya rusak!"</p>
      ...
      <div className="patient3-stage">
        <div
          ...
          style={{ backgroundImage: `url(${phase === 'intro' ? aruBefore : gigiAru})` }}
        >
          {phase === 'intro' && !completed.mirrorProbe && (
            <div className="patient3-stage__hint">Seret mirror + probe untuk memeriksa gusi</div>
          )}
          ...
          {phase === 'exposed' && (
            <>
              {soreAreas.map((area) => (
                soreState[area.id] ? (
                  <div key={`sore-${area.id}`} className="aru-overlay aru-overlay--sore" style={areaStyle(area)} />
                ) : numbState[area.id] ? (
                  <div key={`numb-${area.id}`} className="aru-overlay aru-overlay--numb" style={areaStyle(area)} />
                ) : null
              ))}

              {toothTargets.map((area) => (
                toothState[area.id] ? (
                  <div key={`tooth-${area.id}`} className="aru-overlay aru-overlay--tooth" style={areaStyle(area)} />
                ) : (
                  <div key={`socket-${area.id}`} className="aru-overlay aru-overlay--socket" style={areaStyle(area)} />
                )
              ))}

              {bleedingAreas.map((area) => (
                bleedState[area.id] ? (
                  <div key={`bleed-${area.id}`} className="aru-overlay aru-overlay--bleed" style={areaStyle(area)} />
                ) : (
                  <div key={`dry-${area.id}`} className="aru-overlay aru-overlay--dry" style={areaStyle(area)} />
                )
              ))}
              ...
              {bacteriaAreas.map((area) => (
                bacteriaState[area.id] ? (
                  <img
                    key={area.id}
                    src={area.type === 'porphyromonas' ? porphyromonasImg : candidaImg}
                    alt={area.type}
                    className={`aru-bacteria aru-bacteria--${area.type}`}
                    style={{ ...areaStyle(area), '--aru-rot': `${area.rot}deg` }}
                  />
                ) : null
              ))}
            </>
      ...
      {draggingTool && activeTool && (
        <div className="patient3-dragGhost" style={{ left: toolPos.x, top: toolPos.y }}>
        ...
        <div className="patient3-toolDock__inner">
          {toolSequence.map((tool) => (
            ...
            >
              ...
              {tool.done && <span className="patient3-toolBtn__done">Selesai</span>}
            </button>
          ))}
        </div>
      </div>
      <div className="patient3-dockSpacer" />
    </div>
  );
};
